...
io.on('connection', (socket) => {
    ...
    socket.on('startGame', (data) => {
        console.log("Enviem als jugadors que comencem el joc")
        games.get(parseInt(data.pin)).starting_game = true; // Activa el flag per dir que quan es tanqui el socket per iniciar el joc, el servidor no es pensi que estem sortint del joc
    ...
    socket.on('disconnect', ()=>{
        ...
        for(let pin of games.keys()) {
            ...
            if((game.host_id == socket.id || game.player_id == socket.id) && !game.starting_game) {
                console.log("El host o el jugador s'ha anat");
                games.delete(pin);
